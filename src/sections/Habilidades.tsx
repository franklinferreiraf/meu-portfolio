    ...
    <section id="habilidades" aria-labelledby="habilidades-title" className="py-20 border-t border-cardBorder/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {habilidades.map((categoria) => (
          <div key={categoria.id} className="bg-card border border-cardBorder rounded-2xl p-6 card-hover flex flex-col">
            {/* Título da Categoria com Ícone */}
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-background border border-cardBorder rounded-lg">{categoria.icone}</div>
