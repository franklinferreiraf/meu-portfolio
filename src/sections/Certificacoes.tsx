      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {certificacoes.map((cert) =>
          cert.placeholder ? (
            <li
              key={cert.id}
              className="flex items-center gap-4 p-4 border border-dashed border-cardBorder/80 rounded-xl text-left"
            >
              <div className="w-12 h-12 flex-shrink-0 bg-background border border-cardBorder rounded-lg flex items-center justify-center">
                <svg className="w-6 h-6 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                </svg>
              </div>
              <div>
                <h3 className="text-muted font-medium">{cert.nome}</h3>
                <p className="text-gray-500 text-sm">{cert.emissor}</p>
              </div>
            </li>
          ) : (
            <li
              key={cert.id}
              className="flex items-center gap-4 p-4 bg-card border border-cardBorder rounded-xl card-hover"
            >
